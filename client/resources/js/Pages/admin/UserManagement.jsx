...
const UserManagement = ( { admins } ) => {
    ...
    useEffect(() => {
        const user = admins.filter((admin) => admin.id == id)[0];
        setUser(user);
    }, [id])
  ...
  return (
    <div>
        <DeleteModal isDeleteClicked={isDeleteClicked} setIsDeleteClicked={setIsDeleteClicked} user={user} url="admin/user-management"/>
        <EditData isEditClicked={isEditClicked} setIsEditClicked={setIsEditClicked} user={user} url="admin/user-management"/>
        <CreateAdmin isRegisterClicked={isRegisterClicked} setIsRegisterClicked={setIsRegisterClicked}/>
        <Nav>
                                ...
                                {admins && admins.map((admin, index) => {
                                    return (<tr key={index} className={`${index % 2 == 0 ? 'bg-gray-50' : 'bg-cwhite'} border-b border-gray-200 text-cblack`}>
                                        ...
                                        <td className="px-6 py-4 space-x-6 flex">
                                            <button className='px-6 py-2 bg-clgreen hover:bg-cgreen rounded-lg ctransition' onClick={() => handleEdit(admin.id)}>Edit</button>
                                            <button className='px-6 py-2 bg-red-500 text-cwhite rounded-lg hover:bg-red-600 ctransition' onClick={() => handleDelete(admin.id)}>Delete</button>
                                        </td>
                                    </tr>
                                )})}
                            </tbody>
