...
const Nav = ( ) => {
  ...
  return (
    <motion.div
        ...
        animate={ !noAnimationRoles.includes(role) ? {opacity: 1, transition: { duration: 1.2, delay: 1.3, ease: [0, 0.55, 0.45, 1]}} : {}}
    >
        <AnimatePresence>
      ...
      <div className="desktop__nav w-full flex justify-between px-6 py-6 items-center ctransition">
        ...
        <div className="relative group__sec-2 hidden md:flex md:space-x-2 lg:space-x-6 items-center">
            <div className={`contact ${user && user.role == 'admin' ? 'hidden' : 'block'}`}>
                <a href="#" className='md:text-sm lg:text-lg py-3 px-8 bg-cblack text-white rounded-md border border-cblack hover:bg-clgreen hover:text-cblack ctransition'>Contact</a>
            </div>
            ...
            {user && (
                <div
                    ...
                    <div className={`image-wrapper rounded-full overflow-hidden w-12 h-12 ${!user ? 'hidden' : 'block'} border border-cblack cursor-pointer`}>
                            ...
                            className='absolute right-0 transform top-16 bg-cwhite rounded-lg shadow-lg w-[150px] border border-gray-300'>
                                <ul className='rounded-lg overflow-hidden'>
                                    <li className='w-full hover:bg-cgray ctransition border-b border-gray-300 cursor-pointer'>
                                        <Link href="/profile" className='w-full px-2 py-4 flex items-center justify-between space-x-8'>
                                            <span>Profile</span>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
                                                <path fill="currentColor" d="M6.4 18L5 16.6L14.6 7H6V5h12v12h-2V8.4z"/>
                                            </svg></Link>
                                    </li>
                                    <li className='w-full hover:bg-cgray ctransition cursor-pointer' onClick={(e) =>{e.preventDefault(); post('/logout')}}>
